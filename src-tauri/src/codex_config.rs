...
const CODEX_CUSTOM_HISTORY_ALIAS_HEADER: &str = "x-cc-switch-history-provider-alias";
...
fn codex_provider_table_has_custom_history_alias_marker(item: &Item) -> bool {
    ...
    headers
        .as_inline_table()
        .and_then(|table| table.get(CODEX_CUSTOM_HISTORY_ALIAS_HEADER))
...
fn mark_codex_custom_history_proxy_alias(item: &mut Item) -> Result<(), AppError> {
    let provider = item.as_table_mut().ok_or_else(|| {
        AppError::Message(
            "Cannot mark Codex custom history compatibility alias: Provider is not a table"
                .to_string(),
        )
    })?;
    match provider.get_mut("http_headers") {
        Some(headers) => {
            if let Some(inline) = headers.as_inline_table_mut() {
                inline.insert(
                    CODEX_CUSTOM_HISTORY_ALIAS_HEADER,
                    toml_edit::Value::from(CC_SWITCH_CODEX_MODEL_PROVIDER_ID),
                );
            } else if let Some(table) = headers.as_table_mut() {
                table[CODEX_CUSTOM_HISTORY_ALIAS_HEADER] =
                    toml_edit::value(CC_SWITCH_CODEX_MODEL_PROVIDER_ID);
            } else {
                return Err(AppError::Message(
                    "Cannot mark Codex custom history compatibility alias: http_headers is not a table"
                        .to_string(),
                ));
            }
        }
        None => {
            ...
            provider["http_headers"] = Item::Value(toml_edit::Value::InlineTable(headers));
        }
    }
    Ok(())
}

/// Keep the historical CCSM `custom` session bucket resolvable while another
/// managed takeover Provider is active. Old third-party and unified-history
/// tasks persist this Provider id and Codex validates it before sending a
/// request, so a local alias must coexist with the newer Router/official id.
///
/// A user-owned remote `custom` table is never replaced. Only a missing table
/// or a table already proven to be a CCSM local takeover artifact can be owned
/// by this compatibility projection.
pub(crate) fn install_codex_custom_history_proxy_alias(
    ...
    let should_install = providers
        .get(CC_SWITCH_CODEX_MODEL_PROVIDER_ID)
        .is_none_or(codex_provider_table_has_custom_history_alias_marker);
    if !should_install {
        return Ok(());
    }
    let mut source = providers.get(source_provider_id).cloned().ok_or_else(|| {
        AppError::Message(format!(
            "Cannot create Codex custom history compatibility alias: source Provider `{source_provider_id}` is missing"
        ))
    })?;
    mark_codex_custom_history_proxy_alias(&mut source)?;
    providers.insert(CC_SWITCH_CODEX_MODEL_PROVIDER_ID, source);
    Ok(())
}

fn remove_codex_custom_history_proxy_alias(doc: &mut DocumentMut) {
    let should_remove_container = doc
        .get_mut("model_providers")
        ...
        .map(|providers| {
            if providers
                .get(CC_SWITCH_CODEX_MODEL_PROVIDER_ID)
                .is_some_and(codex_provider_table_has_custom_history_alias_marker)
            {
            ...
            for provider_id in [
                ...
                let should_remove = providers.get(provider_id).is_some_and(|item| {
                    provider_id == CC_SWITCH_CODEX_ROUTER_MODEL_PROVIDER_ID
                        || codex_provider_table_has_proxy_placeholder(item)
                        || codex_provider_table_has_custom_history_alias_marker(item)
                });
                if should_remove {
...
fn strip_codex_provider_owned_fields_from_live(live_config_text: &str) -> Result<String, AppError> {
    ...
    remove_codex_custom_history_proxy_alias(&mut live_doc);
    for key in [
        "model",
        "model_provider",
        "openai_base_url",
...
pub fn apply_codex_official_proxy_route_with_system_proxy_policy(
    ...
    install_codex_custom_history_proxy_alias(
        &mut providers,
        CC_SWITCH_CODEX_OFFICIAL_PROXY_PROVIDER_ID,
    )?;
